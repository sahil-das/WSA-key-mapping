...
from gui_helpers import set_coordinates, edit_key_action, key_to_touch, update_key_buttons, remove_key
...
selected_platform = None  # Global variable for selected platform
...
def create_controller():
    global adb_status_var, adb_status_label, selected_platform
    ...
    selected_platform = StringVar(value="ADB")  # Must be created after root
    ...
    ttk.Label(frame_top, text="Platform:").grid(row=0, column=0, padx=5)
    platform_menu = ttk.Combobox(
        frame_top,
        textvariable=selected_platform,
        values=["ADB", "Windows"],
        state="readonly",
        width=10
    )
    ...
    def update_platform_ui(*args):
        if selected_platform.get() == "Windows":
            connect_btn.grid_remove()
            adb_status_label.grid_remove()
        else:
            connect_btn.grid(row=1, column=0, padx=5, pady=(10, 0))
            adb_status_label.grid(row=1, column=1, padx=5, pady=(10, 0))

    selected_platform.trace_add("write", update_platform_ui)

    ttk.Label(frame_top, text="Key:").grid(row=0, column=2, padx=5)
    key_var = StringVar()
    key_entry = ttk.Entry(frame_top, textvariable=key_var, width=10)
    key_entry.grid(row=0, column=3, padx=5)

    ttk.Label(frame_top, text="Action Type:").grid(row=0, column=4, padx=5)
    action_type_var = StringVar(value="Single Tap")
    action_menu = ttk.Combobox(
        frame_top,
        textvariable=action_type_var,
        values=["Single Tap", "Multiple Taps", "Scroll", "Long Press", "Swipe", "Continuous Click"],
        state="readonly"
    )
    action_menu.grid(row=0, column=5, padx=5)
    ...
    def add_key():
        ...
        elif selected_type == "continuous click":
            key_to_touch[key] = {"type": "continuous_click", "x": 0, "y": 0}
    ...
    ttk.Button(frame_top, text="Add", command=add_key).grid(row=0, column=6, padx=5)
    ttk.Button(frame_top, text="Reset", command=reset_fields).grid(row=0, column=7, padx=5)
    ...
    connect_btn = ttk.Button(frame_bottom, text="Connect ADB", command=lambda: connect_adb(force=True))
    connect_btn.grid(row=1, column=0, padx=5, pady=(10, 0))
